...
#' @param right.title right title of x axis
#' @param left.title left title of x axis
...
pyramid <- function(data,left,right,groupID,groupName)
{
  ...
  mg_1 <<- unit(c(0, 0.0, 0.3, 0.5), "lines")  
  ...
  p_1 <- ggplot(test_data) +
    ...
    theme(axis.title.x = element_text(family = Fon, size = left.axis.title.x.size),
  ...
  p_2 <- ggplot(test_data) +
    ...
    theme(axis.title.x = element_text(family = Fon, size = right.axis.title.x.size),
          axis.title.y = element_text(family = Fon, size = 30)) +
